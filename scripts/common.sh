...
while [ $# -gt 0 ]; do
    case "$1" in
        ...
        --project-version-suffix)
            echo "option: $1"
            PROJECT_VERSION_SUFFIX="$2"
            shift
            ;;
        --project-version-codename)
            echo "option: $1"
            PROJECT_VERSION_CODENAME="$2"
            shift
            ;;
        --project-branch)
            echo "option: $1"
...
EXTRA_CMAKE_VARIABLES=(
    ...
    "-DDEFAULT_PROJECT_VERSION_SUFFIX:STRING=${PROJECT_VERSION_SUFFIX}"
    "-DDEFAULT_PROJECT_VERSION_CODENAME:STRING=${PROJECT_VERSION_CODENAME}"
...
EXTRA_CMAKE_VARIABLES+=("${EXTRA_ARGS[@]}")
