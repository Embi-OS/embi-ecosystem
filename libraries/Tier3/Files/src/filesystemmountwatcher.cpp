...
public:
    ...
    void attach(FileSystemMountWatcher *watcher)
    {
...
#  endif
    ...
    void mountsChanged()
    {
        auto currentMounts = currentMountPoints();

        if(!m_watchers.isEmpty()) {
            // Check for additions
            for(auto [key, value]: currentMounts.asKeyValueRange()) {
                if(!m_mounts.contains(key, value)) {
                    for (auto watcher: std::as_const(m_watchers))
                        emit watcher->mountAdded(key, value);
                }
            }
            // Check for removals
            for(auto [key, value]: m_mounts.asKeyValueRange()) {
                if(!currentMounts.contains(key, value)) {
                    for (auto watcher: std::as_const(m_watchers))
                        emit watcher->mountRemoved(key, value);
                }
            }
        }

        if (!m_mountPoints.isEmpty()) {
            for(auto [key, value]: m_mountPoints.asKeyValueRange()) {
                bool isMounted = currentMounts.contains(key);
                bool wasMounted = m_mounts.contains(key);

                if (isMounted != wasMounted)
                    emit value->mountChanged(key, currentMounts.value(key));
            }
        }
...
private:
    ...
    QList<FileSystemMountWatcher *> m_watchers;
