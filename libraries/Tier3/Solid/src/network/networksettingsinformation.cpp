...
bool NetworkSettingsInformation::waitForReachability(QNetworkInformation::Reachability reachability, int timeout)
{
    ...
    {
        NETWORKLOG_WARNING()<<"NetworkSettingsInformation reachability can not be determined";
        return true;
    }

    if(netInfo()->reachability()>=reachability)
    {
        NETWORKLOG_INFO()<<"NetworkSettingsInformation reachability is"<<netInfo()->reachability();
        return true;
    }
    ...
    auto conn3 = connect(&debug, &QTimer::timeout, netInfo(), [&]() {
        NETWORKLOG_INFO()<<"Wait for network reachability - remaining time:"<<qRound(timer.remainingTime()/1000.0)<<"sec";
    });
    ...
    if(result==0)
        NETWORKLOG_INFO()<<"NetworkSettingsInformation reachability changed to"<<netInfo()->reachability()<<"after"<<elapsed.nsecsElapsed()/1000000.0<<"ms";
    else
        NETWORKLOG_WARNING()<<"NetworkSettingsInformation reachability is still"<<netInfo()->reachability()<<"after"<<elapsed.nsecsElapsed()/1000000.0<<"ms";
