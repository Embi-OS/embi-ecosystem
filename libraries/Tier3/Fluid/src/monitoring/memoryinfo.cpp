...
bool extractBytes(quint64 &value, const QByteArray &buffer, const QByteArrayView &beginPattern, qsizetype &from)
{
    ...
    if (from) { // Wrong order? Restart from the beginning
        FLUIDLOG_WARNING()<<"MemoryInfo: extractBytes: wrong order when extracting"<<beginPattern;
        from = 0;
...
bool MemoryInfo::refresh()
{
    ...
    quint64 availPhys = freePhys;
    qsizetype availFrom = 0;
    extractBytes(availPhys, meminfo, "MemAvailable:", availFrom);
    quint64 buffers = 0;
    ...
    const quint64 reclaimableCache = cached + sReclaimable;
    m_cached = reclaimableCache>=sharedMem ? reclaimableCache-sharedMem : 0;
    m_buffers = buffers;
...
bool MemoryInfo::refresh()
{
    FLUIDLOG_WARNING()<<"MemoryInfo: unsupported platform!";
    return false;
}
