    ...
    QAbstractListModel(parent)
{
    ...
    connect(timer, &QTimer::timeout, this, &NetworkInterfaceModel::queueRefresh);
...
QVariant NetworkInterfaceModel::data(const QModelIndex &index, int role) const
{
    ...
    case Address: {
        const QString address = NetworkInterfaceHelper::primaryIpv4Address(interface);
...
void NetworkInterfaceModel::queueRefresh()
{
    if (m_delayed) {
        if (!m_refreshQueued) {
            m_refreshQueued = true;
            QMetaObject::invokeMethod(this, &NetworkInterfaceModel::refresh, Qt::QueuedConnection);
        }
    } else {
        refresh();
    }
