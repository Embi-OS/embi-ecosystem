...
#define deferWarning QMessageLogger(QT_MESSAGELOG_FILE, QT_MESSAGELOG_LINE, QT_MESSAGELOG_FUNC,"DEFER").warning
#define deferInfo QMessageLogger(QT_MESSAGELOG_FILE, QT_MESSAGELOG_LINE, QT_MESSAGELOG_FUNC,"DEFER").info
#define deferDebug QMessageLogger(QT_MESSAGELOG_FILE, QT_MESSAGELOG_LINE, QT_MESSAGELOG_FUNC,"DEFER").debug
#define deferTrace QMessageLogger(QT_MESSAGELOG_FILE, QT_MESSAGELOG_LINE, QT_MESSAGELOG_FUNC,"\x1B""DEFER").debug
...
protected:
    ...
    static QHash< QThread *, QDeferredProxyObject * > s_threadMap;
...
void QDeferredData<Types...>::resolve(QDeferred<Types...> ref, Types(&...args))
{
    ...
    {
        deferWarning()<<"Cannot resolve already processed deferred object - state:"<<m_state;
        return;
    }
    // change state
...
void QDeferredData<Types...>::reject(QDeferred<Types...> ref, Types(&...args))
{
    ...
    if (m_state != QDeferredState::PENDING)
    {
        deferWarning()<<"Cannot reject already processed deferred object - state:"<<m_state;
        return;
    }
    // change state
...
void QDeferredData<Types...>::notify(QDeferred<Types...> ref, Types(&...args))
{
    ...
    if (m_state != QDeferredState::PENDING)
    {
        deferWarning()<<"Cannot notify already processed deferred object - state:"<<m_state;
        return;
    }
